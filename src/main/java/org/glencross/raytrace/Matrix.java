...
public class Matrix {
    ...
    public static Matrix getRotationMatrix(Vector fromVector, Vector toVector) {
        ...
        if (Math.abs(Math.PI-theta) < 0.00001) {
            x = fromVector.crossProduct(new Vector(toVector.getX()*2+1, toVector.getY()*-2-1, toVector.getZ())).toUnit();
        }
    ...
    public static Matrix rotateAroundVector(Vector axis, double radians) {
        Vector xaxis = new Vector(1, 0, 0);
        Matrix result = getRotationMatrix(axis, xaxis)
                .multiply(rotateAroundXAxis(radians))
                .multiply(getRotationMatrix(xaxis, axis));
        return result;

    }

    public static void main(String[] args) {
        System.out.println(rotateAroundYAxis(30));
        System.out.println(rotateAroundVector(new Vector(0, 1, 0), 30));
    }
